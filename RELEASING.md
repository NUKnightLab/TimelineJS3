...
1. Update `CHANGELOG` to indicate the release date under the "not yet released" version header. Hopefully you've been incrementally updating the file with relevant changes so that this is just editing the version "header". Commit this change (`npm version` refuses to run unless the working directory, including the index, is fully clean, so this can't just be staged).
1. Run `npm version <newversion>` (e.g. `npm version 3.9.13`, or `npm version major`/`minor`/`patch`). This bumps `package.json` and `package-lock.json`, commits that bump, and creates a matching git tag. The repo's `.npmrc` configures `npm version` to tag as `3.9.13` rather than the npm default `v3.9.13`, matching our existing tags.
